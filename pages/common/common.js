...
var common = window.common || (function(exports, undefined) {
    ...
    exports.ajax = function(paramsData, successCallback, errorCallback, options) {
        var data = {
            params: encodeURIComponent(JSON.stringify(paramsData.data))
        };

        if (paramsData.isEncrypt != undefined && paramsData.isEncrypt == false) {
            var isEncrypt = false;
        } else {
            isEncrypt = Config.customize.isEncrypt ? Config.customize.isEncrypt : '';
        }
        // 开启调试
        if (options.isDebug) {
        ...
        var ajaxBody = {
            ...
            data: data,
            timeout: paramsData.timeout || 10000,
    ...
    /**
     * 业务功能的附件上传云文档
     * @param {String} attachguid 附件guid
     * @param {String} frommark 附件的业务来源
     * @param {String} fromguid 业务guid
     * @param {String} fromtitle 业务title
     * @param {Function} callback 回调
     */
    exports.businessUpload = function(attachguid, frommark, fromguid, fromtitle, callback) {
        ejs.ui.showWaiting();

        Util.ajax({
            url: Config.serverOA9Url + 'netharddisk_transferfile_v7',
            data: {
