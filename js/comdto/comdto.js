...
(function (exports, controlMap) {
    ...
    extend(List, MControl, {
        ...
        _init: function () {
            ...
            if (this.pageSize > 0) {
                var container = epm.closest(this.el, 'minirefresh-wrap');
                // 未配置下拉刷新的容器，则自动将.mui-content设为下拉容器
                if (!container.id) {
                    container.id = epm.generateId('minirefresh');
                }

                this.container = container;
                // this.scrollId = container.querySelector('ul').id;
                // 绑定上拉加载事件
        ...
        setUrl: function (url) {
            this.url = url;
            if (this.minirefresh) {
                this.minirefresh.options.url = url;
            }
        },
        ...
        initPullRefresh: function () {
            ...
            var successRequestCallback = function (data, isRefresh) {
                ...
                if(total <= self.pageSize * (self.pageIndex + 1)) {
                    self.minirefresh.isNoMoreData = true;
                }
            };
            ...
            var init = function () {
                self.minirefresh = new MiniRefreshBiz({
                    isDebug: true,
            ...
            if (!window.MiniRefreshBiz) {
                Util.loadJs(
                    [
                        Config.bizRootPath + 'js/widgets/minirefresh/minirefresh.css',
                        Config.bizRootPath + 'js/widgets/minirefresh/minirefresh.js',
                    ],
                    Config.bizRootPath + 'js/widgets/minirefresh/minirefresh.bizlogic.js', init);
            } else {
                init();
            }
        },
