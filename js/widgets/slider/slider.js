...
(function (doc, Util) {
    ...
    function Slider(options) {
        ...
        if (self.container.Slider) {
            // 销毁上一个实例的设置
            self.container.Slider.destroy();
        }
        self.container.Slider = self;
        self.moveEle = self.container.querySelector('[data-slider]');

        if (!self.moveEle) {
            self.moveEle = self.container.children[0];
        }
        // self.oldEle = self.moveEle.innerHTML;
        ...
        // 获取方向 执行动画
        self._getDirection(self.direction);
    }
    ...
    Slider.prototype = {
        ...
        destroy: function () {
            ...
            self.height = null;
            self.intev = null;
            // self.moveEle.innerHTML = self.oldEle;
            self.moveEle = null;
        ...
        refresh: function (options) {
            ...
            if (options) {
                ...
                self.intev = options.intev || self.intev;
                ...
                if (options.height) {
                    ...
                    self.height = self.height || self.container.offsetHeight;
                }
            }
            clearTimeout(self.timeOut);
            self.transform(self.moveEle, 0, 0);
            // 获取方向 执行动画
            self._getDirection(self.direction);
        },
